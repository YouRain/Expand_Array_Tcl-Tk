...
proc main_menu {} {
	...
	labelframe $tl.offset -text "Angle of rotation (degree)" -labelanchor nw -font 14 -borderwidth 0 -relief groove
...
proc plus_angle {znak} {
	global file_dir dir file_name delimiter_in delimiter_out name_new_file offset_entry exist_columns tl file_path
	set original_file [open $file_dir r]
	...
	while {[eof $original_file] == 0} {
		...
		set line [join $line "$delimiter_out"]
		append content $line "\n"
	}
	set content [string map {" " ""} $content]
	puts $new_file $content
...
proc solve_column {} {
	global file_dir delimiter_in exist_columns tl
	set exist_columns ""
	set original_file [open $file_dir r]
	set line [gets $original_file]
	set line [split $line "$delimiter_in"]
...
proc help_menu {} {
	...
	set text_help "ƒанна€ программа принимает массив, состо€щий минимум из 2 векторов\(колонок\), которые представл€ют из себ€ координаты точек по ос€м X и Y в декартовой системе координат. ѕрограмма при выборе соответствующей настройке может помен€ть разделитель колонок или добавить массив точек на указанный угол вдоль оси Z. –азделителем целой и дробной частей числа должна быть точка. ‘айл не должен содержать шапки или предислови€."
